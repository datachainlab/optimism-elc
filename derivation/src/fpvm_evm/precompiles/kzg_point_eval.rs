...
pub(crate) fn fpvm_kzg_point_eval<T: PreimageOracleClient + HintWriterClient>(
    input: &[u8],
    gas_limit: u64,
    oracle_reader: &T,
) -> PrecompileResult {
...
mod test {
    ...
    #[test]
    fn test_accelerated_kzg_point_eval() {
        ...
        let accelerated_result = fpvm_kzg_point_eval(&input, u64::MAX, &oracle).unwrap();
        let native_result =
            execute_native_precompile(KZG_POINT_EVAL_ADDR, input, u64::MAX).unwrap();
    ...
    #[test]
    fn test_accelerated_kzg_point_eval_out_of_gas() {
        let oracle = TestOracleReader::new();
        let accelerated_result = fpvm_kzg_point_eval(&[], 0, &oracle).unwrap_err();
        assert!(matches!(accelerated_result, PrecompileError::OutOfGas));
    }

    #[test]
    fn test_accelerated_kzg_point_eval_bad_input_size() {
        let oracle = TestOracleReader::new();
        let accelerated_result = fpvm_kzg_point_eval(&[], u64::MAX, &oracle).unwrap_err();
        assert!(matches!(
            accelerated_result,
