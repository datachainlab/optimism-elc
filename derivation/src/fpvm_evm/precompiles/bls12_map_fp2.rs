...
pub(crate) fn fpvm_bls12_map_fp2<T: PreimageOracleClient + HintWriterClient>(
    input: &[u8],
    gas_limit: u64,
    oracle_reader: &T,
) -> PrecompileResult {
...
mod test {
    ...
    fn test_accelerated_bls12_381_map_fp_g2() {
        ...
        let accelerated_result = fpvm_bls12_map_fp2(&input, 23800, &oracle).unwrap();
        let native_result = execute_native_precompile(
            *bls12_381::map_fp2_to_g2::PRECOMPILE.address(),
            input,
            23800,
        )
        .unwrap();
    ...
    #[test]
    fn test_accelerated_bls12_381_map_fp_g2_bad_gas_limit() {
        let oracle = TestOracleReader::new();
        let accelerated_result = fpvm_bls12_map_fp2(&[], 0, &oracle).unwrap_err();
        assert!(matches!(accelerated_result, PrecompileError::OutOfGas));
    }

    #[test]
    fn test_accelerated_bls12_381_map_fp_g2_bad_input_size() {
        let oracle = TestOracleReader::new();
        let accelerated_result = fpvm_bls12_map_fp2(&[], u64::MAX, &oracle).unwrap_err();
