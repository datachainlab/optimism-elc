...
pub mod testtool {
    ...
    impl L2Client {
        ...
        pub fn chain_id(&self) -> Result<u64> {
            ...
            let result: RpcResult<String> = response.json()?;
        ...
        pub  fn rollup_config(&self) -> Result<RollupConfig> {
            ...
            let response = self.check_response(response)?;
            let result: RpcResult<RollupConfig> = response.json()?;
            Ok(result.result)
        }
        ...
        pub fn output_root_at(&self, number: u64) -> Result<OutputRootAtBlock> {
            let client = reqwest::blocking::Client::new();
            let body = RpcRequest {
