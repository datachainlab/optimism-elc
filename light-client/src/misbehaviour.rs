...
fn get_game_uuid(source_game_type: u64, l2_block_num: B256, output_root: B256) -> B256 {
    ...
    let extra_len = u64_to_bytes32(l2_block_num.len() as u64);
...
impl FaultDisputeGameProof {
    fn get_game_id(
        ...
        let game_uuid = get_game_uuid(
            ...
            B256::from(u64_to_bytes32(claimed_l2_number)),
            claimed_output_root,
        );
...
impl<const SYNC_COMMITTEE_SIZE: usize> L2FutureMisbehaviour<SYNC_COMMITTEE_SIZE> {
    pub fn verify(
        ...
        if consensus_l2_number >= self.resolved.resolved_l2_number {
            return Err(Error::UnexpectedMisbehaviourHeight(
                consensus_l2_number,
                self.resolved.resolved_l2_number,
            ));
        }
...
mod test {
    ...
    use crate::misbehaviour::{
        ...
        L2HeaderHistory, L2Misbehaviour,
    };
    ...
    fn test_try_from_l2_misbehaviour_error() {
        let base = RawL2Misbehaviour {
            ...
            resolved_l2_number: 0,
            resolved_output_root: [0u8; 32].into(),
        ...
        let err = L2Misbehaviour::<32>::try_from(raw).unwrap_err();
        match err {
            Error::UnexpectedResolvedL2Number(_, _) => {}
            _ => panic!("Unexpected error, got: {:?}", err),
        }
