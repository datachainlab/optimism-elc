...
use optimism_ibc_proto::ibc::lightclients::optimism::v1::FaultDisputeGameFactoryProof as RawFaultDisputeGameFactoryProof;
...
#[derive(Clone, Debug)]
pub struct FaultDisputeGameFactoryProofAttribute {
...
pub struct FaultDisputeGameFactoryProof<const SYNC_COMMITTEE_SIZE: usize> {
    ...
    /// Proof of DisputeGameFactoryProxy
    attribute: FaultDisputeGameFactoryProofAttribute,
}

impl<const SYNC_COMMITTEE_SIZE: usize> TryFrom<RawFaultDisputeGameFactoryProof>
    for FaultDisputeGameFactoryProof<SYNC_COMMITTEE_SIZE>
{
    type Error = Error;

    fn try_from(value: RawFaultDisputeGameFactoryProof) -> Result<Self, Self::Error> {
        let l1_header =
            L1Header::try_from(value.l1_header.ok_or(Error::proto_missing("l1_header"))?)?;
        let dispute_game_factory_address =
            Address::try_from(value.dispute_game_factory_address.as_slice())
        ...
        let dispute_game_factory_storage_proof =
            decode_eip1184_rlp_proof(value.dispute_game_factory_storage_proof)?;
        ...
        let fault_dispute_game_storage_proof =
            decode_eip1184_rlp_proof(value.fault_dispute_game_storage_proof)?;
        ...
        Ok(Self {
            ...
            attribute: FaultDisputeGameFactoryProofAttribute {
                ...
                dispute_game_factory_account,
                dispute_game_factory_storage_proof,
                fault_dispute_game_account,
                fault_dispute_game_storage_proof,
...
impl<const SYNC_COMMITTEE_SIZE: usize> FaultDisputeGameFactoryProof<SYNC_COMMITTEE_SIZE> {
    ...
    pub fn get_game_id(
        ...
        self.attribute
            .dispute_game_factory_account
            .verify_account_storage(&self.attribute.dispute_game_factory_address, state_root)?;

        // Extract game id from DisputeGameFactoryProxy by output_root.
        let game_uuid = get_game_uuid(
            self.attribute.fault_dispute_game_source_game_type,
        ...
        execution_verifier
            .verify(
                ...
                self.attribute.dispute_game_factory_storage_proof.clone(),
            )
            .map_err(|err| Error::UnexpectedDisputeGameFactoryProxyProof {
                proof: self.attribute.clone(),
        ...
        let packing_slot_value = execution_verifier
            .verify(
                self.attribute
                    ...
                    .account_storage_root,
                status_key.as_slice(),
                self.attribute.fault_dispute_game_storage_proof.clone(),
            )
    ...
    pub fn verify_game_not_created(
        &self,
        fault_dispute_game_config: &FaultDisputeGameConfig,
        claimed_l2_number: u64,
        claimed_output_root: B256,
        ...
        if let Some(game_id) = game_id {
            return Err(Error::UnexpectedGameExists(game_id));
        }
        Ok(())
    }
    ...
    fn try_from(raw: RawL2Misbehaviour) -> Result<Self, Self::Error> {
        ...
        if submitted_l1_proof.is_some() && submitted_l1_parent_proof.is_some() {
            ...
            let parent = submitted_l1_parent_proof
                ...
                .block_number;
            if target != parent + 1 {
                return Err(Error::UnexpectedSealedL1Relation(target.0, parent.0));
            }
...
mod test {
    ...
    use crate::misbehaviour::{
        ...
        FaultDisputeGameFactoryProofAttribute, HeaderWithMessagePasserAccount, L2HeaderHistory,
    };
    use alloc::vec;
    use alloc::vec::Vec;
    use alloy_consensus::Header;
    ...
    fn test_verify_resolved_status_invalid_factory_account() {
        ...
        model.attribute.dispute_game_factory_account.account_proof = vec![];
    ...
    fn test_verify_resolved_status_error_invalid_game_proof() {
        let mut model = default_fault_dispute_game_factory_proof();
        model.attribute.fault_dispute_game_storage_proof.pop();
        let claim = default_valid_claim();
        let err = model
            .verify_resolved_status(&FaultDisputeGameConfig::default(), claim.0, claim.1)
            .unwrap_err();
    ...
    fn test_verify_resolved_status_invalid_game_account() {
        let mut model = default_fault_dispute_game_factory_proof();
        model.attribute.fault_dispute_game_account.account_proof = vec![];
