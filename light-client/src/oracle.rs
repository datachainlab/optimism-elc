...
fn verify_precompile_and_blob_preimage(key: &PreimageKey, data: &[u8], preimages: &HashMap<PreimageKey, Vec<u8>>) -> Result<(), Error> {
    match key.key_type() {
        PreimageKeyType::Precompile => {
            let raw_key = B256::from(*key).0;
            let hash_code_key = PreimageKey::new(raw_key, PreimageKeyType::Keccak256);
            let hint_data = preimages.get(&hash_code_key)
                .ok_or(Error::NoPreimagePrecompiledCodeFound {
                    key: hash_code_key
                })?;
