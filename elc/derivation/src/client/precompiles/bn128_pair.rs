...
pub(crate) const FPVM_ECPAIRING_GRANITE: PrecompileWithAddress = PrecompileWithAddress(
    ECPAIRING_ADDRESS,
    Precompile::Standard(fpvm_ecpairing_granite),
);
...
fn fpvm_ecpairing(input: &Bytes, gas_limit: u64) -> PrecompileResult {
    ...
    let result_data = precompiles::execute(ECPAIRING_ADDRESS, input.to_vec())
